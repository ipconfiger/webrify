...
<script type="module">
  ...
  mount(document.getElementById("webrify-ts"), {
    ...
    onVerify: (token) => {
      // 把 token 发到【你自己的后端】。
      // token 是 HS256 签名、绑定 origin、短时效的 JWT——你的后端可以：
      //   (a) 信任它的签名（共享 jwt_key），或
      //   (b) 调用将来的 /siteverify 端点让 webrify 复核
...
```tsx
import { TurnstileWidget } from "webrify-turnstile"; // 若作为 npm 包发布
<TurnstileWidget
...
```
浏览器（受保护页）                   webrify server                      Redis
     │                                    │                               │
     │  1. POST /challenge (Origin头) ────▶│ 校验 Origin ∈ allowed_origins  │
     │                                    │ 生成 challenge seed + salt      │
     │                                    │ HMAC 签名全部参数 ────────────▶│ （无状态签名，不存）
     │  ◀──── Challenge {algo,salt,        │                                │
     ...
     │     - 收集行为 → WASM CV 评分                                         │
     │     - seed = challenge ‖ 指纹                                          │
     │     - solve_bounded(seed, difficulty, maxnumber) → nonce              │
     │                                                                       │
     │  3. POST /verify {nonce, 指纹, 行为评分, ...} ──▶│                      │
     │                                    │ ① 验 HMAC（constant-time）       │
     │                                    │ ② 查未过期                       │
     │                                    │ ③ 原子标记已花 ────────────────▶│ SET NX EX
     │                                    │ ④ 重算 SHA-256(seed‖nonce) 验前导零
     │                                    │ ⑤ nonce ≤ maxnumber              │
     │                                    │ ⑥ origin ∈ allowed_origins       │
     │                                    │ ⑦ 风险评分（指纹+行为+solve-time）│
     │                                    │    Escalate/Deny → 记录升级 ───▶│ INCR (下次提难度)
     │                                    │ ⑧ 签发 JWT                        │
...
- **可继续**：`/siteverify` 端点（供依赖方后端复核 token）、Ed25519 JWT 升级、分布式 Redis、Grafana 仪表盘、sitekey 热加载
